...
void Protocole::extraireCommande(){
...
void Protocole::tramerMission(Message* message, int nbrePaquets, int numPaquet){
	...
	//Pour cet incrément, seule la mission température peut être lancée
	if (message->getTypeMission()==TypeMisEtat::TEMPCELSIUS)
	{
		laListe=message->getMesures();

		/*************** Traitement de la première donnée ********************/
		tableau[j]=' ';
		j++;
		...
		lamesure = *lIndice; //On met dans la variable lamesure le contenu de l'itérateur
		...
		laListe.pop_front(); //On supprime la donnée traitée

		/*************** Traitement de la deuxième donnée ********************/
		tableau[j]=' ';
		j++;
		strcpy(tab, TypeMisEtat::TEMPERATURE.c_str());
		for(unsigned int k = 0 ; k< TypeMisEtat::TEMPERATURE.size() ;k++)
	...
	/** Mise à jour du champ "nombre d'octets" **/
